...
/**
 * GET /api/officer/cases/my/active
 * Returns active cases assigned to the authenticated officer
...
export async function GET(request: NextRequest) {
  try {
    ...
    // Constrain to this officer and active status
    const assignedFilter = { assignedOfficer: payload.userId, status: 'active' };
    ...
    console.error('[My Active Cases] Error:', error);
    return NextResponse.json({ success: false, error: 'Failed to fetch active cases' }, { status: 500 });
  }
}
